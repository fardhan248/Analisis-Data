...
c
      call DDX4 (p,p4,l,dz)
c
c      OPEN(UNIT=12, FILE='data.txt')
c   
c900   format ('p(z)',',','dp/dz: (analytical)',',','(forward)',',',
c    &'(backward)',',','(2nd order)',',','(4th order)')
c 901   format (f8.2, ',', f9.5, 4(',', f9.5))
c
     ! write (10,900)                                                           
c     
c      write(10,"(2x, 'p(z)')")
      do 2104 k = 1, l                      
        write(12,1001)z(k),p(k),anal(k),p1f(k),p1b(k), p2(k),p4(k)
c                write(10,"(F8.2)") p(k)               
 2104 continue
c                                                                 
1000  format (5x,'p(z)',4x,'dp/dz: (analytical)',4x,'(forward)',7x,'(back&
     &ward)',5x,'(2nd order)',5x,'(4th order)')                                                         
1001  format (f7.1,2x,f8.2,10x,f9.5,4(7x,f9.5))                            
      stop                                                                     
      end                                                                      
